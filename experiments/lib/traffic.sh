...
#
#   Data plane:    10 parallel pings via uesimtun0..9 -> 8.8.8.8 through UPF.
#   Control plane: 4 UEs deregister/register every 15s exercising the full
...
start_traffic() {
    ...
    if [[ -z "$UE_POD" ]]; then
        UE_POD=$(kubectl get pods -n open5gs --no-headers 2>/dev/null \
    ...
    # 1. Data plane: continuous pings on every available uesimtun interface.
    ...
    # 2. Control plane: cycle 4 UEs through deregister/register every 15s.
    (

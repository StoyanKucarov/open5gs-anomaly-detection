#!/usr/bin/env python3
"""
models/logs/plot_results.py

Generates all visualizations from a saved eval_results.json produced by
evaluate.py.  Re-run this script freely without re-training any models.

Outputs (models/logs/out/)
--------------------------
  eval_f1_heatmap.png      — best-threshold F1 per fault × model
...
-----
  ...
  --results PATH   path to eval_results.json
                   (default: models/logs/out/eval_results.json)
"""
...
# Canonical class per slug (source of truth: analysis/lib.py EXPERIMENTS).
# Overrides whatever fault_class is stored in the JSON so old eval outputs
...
def plot_heatmap(results: list[dict], metric: str,
    ...
    ax.set_title(f"Per-Fault {label_str} — Log Anomaly Detection\n"
...
def write_summary(results: list[dict], model_names: list[str],
    ...
    for m in model_names:
        header += f"  {m+' F1':>{col_w}} {m+' AUC':>{col_w}}"

    lines = ["=" * len(header), header, "-" * len(header)]
    prev_class = None

    # Sort by fault_class then slug; fall back gracefully if a model is absent.
    ...
    for slug in sorted(by_slug, key=lambda s: (
        ...
        for m in model_names:
            mr    = row.get(m, {})
            f1    = mr.get("avg_precision", 0.0)
            auroc = mr.get("auroc",        0.5)
            line += f"  {f1:>{col_w}.3f} {auroc:>{col_w}.3f}"
        lines.append(line)
...
def apply_heldout_deeplog(results: list[dict],
                          heldout_path: Path) -> tuple[list[dict], list[str]]:
    """
    Replace DeepLog entries for held-out test-split faults with scores from
    the held-out evaluation file.  Returns (patched_results, heldout_slugs).
    """
    heldout_results, _, heldout_meta = load_results(heldout_path)
    test_slugs = set(heldout_meta.get("test_slugs", []))
    if not test_slugs:
        # Fall back: use whatever slugs appear in the heldout file
        test_slugs = {r["slug"] for r in heldout_results}

    heldout_by_slug: dict[str, dict] = {
        r["slug"]: r for r in heldout_results if r["model"] == "DeepLog"
    }

    patched = []
    for r in results:
        if r["model"] == "DeepLog" and r["slug"] in heldout_by_slug:
            patched.append(heldout_by_slug[r["slug"]])
        else:
            patched.append(r)

    return patched, sorted(test_slugs)
...
def parse_args():
    ...
    ap.add_argument(
        "--heldout", type=Path, default=None,
        help="Held-out eval_results_heldout.json — replaces DeepLog scores "
             "for the test-split faults with the held-out evaluation values.",
    )
...
def main():
    ...
    if args.heldout is not None:
        if not args.heldout.exists():
            raise FileNotFoundError(f"{args.heldout} not found.")
        results, heldout_slugs = apply_heldout_deeplog(results, args.heldout)
        n = len(heldout_slugs)
        print(f"  Patched DeepLog for {n} held-out faults: "
              + ", ".join(s.split("-")[0] for s in heldout_slugs))
        suffix = "_heldout"

    plot_heatmap(results, "avg_precision", model_names,
                 OUT / f"eval_f1_heatmap{suffix}.png")
    plot_heatmap(results, "auroc",         model_names,
                 OUT / f"eval_auroc_heatmap{suffix}.png")
    plot_heatmap(results, "best_recall",   model_names,
                 OUT / f"eval_recall_heatmap{suffix}.png")
    write_summary(results, model_names,
                  OUT / f"eval_per_fault_summary{suffix}.txt")
